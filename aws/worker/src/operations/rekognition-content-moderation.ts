...
import { StartContentModerationRequest } from "aws-sdk/clients/rekognition";

const configVariables = ConfigVariables.getInstance();

export async function contentModeration(providers: ProviderCollection, jobAssignmentHelper: ProcessJobAssignmentHelper<AIJob>, ctx: WorkerContext) {
    ...
    const params: StartContentModerationRequest = {
        Video: {
            S3Object: {
                Bucket: outputBucket,
                Name: tempKey,
    ...
    const data = await ctx.rekognition.startContentModeration(params).promise();
