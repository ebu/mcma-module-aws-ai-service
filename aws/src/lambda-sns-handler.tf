...
locals {
  lambda_name_sns_handler = format("%.64s", replace("${var.prefix}-sns-handler", "/[^a-zA-Z0-9_]+/", "-" ))
  sns_handler_zip_file    = "${path.module}/lambdas/sns-handler.zip"
}

resource "aws_iam_role" "sns_handler" {
  name = format("%.64s", replace("${var.prefix}-${var.aws_region}-sns-handler", "/[^a-zA-Z0-9_]+/", "-" ))
...
resource "aws_iam_role_policy" "sns_handler" {
  name = aws_iam_role.sns_handler.name
  role = aws_iam_role.sns_handler.id
    ...
    Statement = concat([
      ...
      },
      {
        ...
        Resource = concat([
          ...
          "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${local.lambda_name_sns_handler}:*",
...
resource "aws_lambda_function" "sns_handler" {
  depends_on = [
    aws_iam_role_policy.sns_handler
  ]

  function_name    = local.lambda_name_sns_handler
  role             = aws_iam_role.sns_handler.arn
  handler          = "index.handler"
  filename         = local.sns_handler_zip_file
  source_code_hash = filebase64sha256(local.sns_handler_zip_file)
  runtime          = "nodejs18.x"
  timeout          = "30"
  memory_size      = "2048"
